...
def calculate_energies(input_dir='relax-d4-lcao-2/', output_file='results_ads_eners_spinPol-V2.csv'):
    """
    ...
    with paropen(output_file, "w") as fout:
        ...
        for punto, indice_ in puntos.items():
            for pos in positions:
                ...
                st_mol = st.copy()
                calc_mol = GPAW(**calc_dic,
                                setups=get_ghost_setups(indice_, target='ghost'),
                                txt=f'logs/{label}-etotal-mol-spinPol.log')
                ...
                calc_surf = GPAW(**calc_dic,
                                 ...
                                 txt=f'logs/{label}-etotal-surf-spinPol.log')
                ...
                calc_total = GPAW(**calc_dic,
                                  txt=f'logs/{label}-etotal-spinPol.log')
